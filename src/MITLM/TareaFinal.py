    ...
    for root, directories, files in os.walk(target):
        for fileName in files:
            ...
            for sentence in sentences:
                ...
                corpus.append(filter(lambda word: word.isalnum(), words))
    ...
    def rate(self, model, _numberOfNGram):
        ...
        if len(self.tweet_content)>0:
            with open('TEMP', 'w') as fileName:
                fileName.write(self.tweet_content)

            pipe = os.popen('mitlm/evaluate-ngram -o ' +str(_numberOfNGram)+ ' -l ' + model +'  -ep ./TEMP | tail -n 1 | cut -f 4')
            #res = subprocess.call(['mitlm/evaluate-ngram', '-o', str(_numberOfNGram) ,'-l', model, '-ep', 'temporal'])
                
            self.rating = float(pipe.readline())
            os.remove('TEMP')
...
def processSource (target, tweets, _numberOfNGram):
    ...
    #model = NgramModel(_numberOfNGram, corpus, False, True, None )
    model = target+'.'+str(_numberOfNGram)+'.lm'
    #subprocess.call(['./mitlm/estimate-ngram', '-order',str(_numberOfNGram), '-t', target+'.txt', '-wl', model])   
...
#        corpus = createCorpus ('./CORPUS/DOCUMENTS/'+target)
#        with open (target+".txt", 'w')as fileExt:
#            for i in corpus:
#                fileExt.write(' '.join(i)+'\n')
            
        tweets = loadTweets (target)
#        with open (target+"TWEET.txt", 'w') as fileExt:
#            for t in tweets:
#                t.clean()
#                fileExt.write(t.tweet_content+'\n')

        returnedValues = []
        for i in xrange(2,6):
            returnedValues.append(evaluateCharacterization(processSource(target, tweets, i)))
